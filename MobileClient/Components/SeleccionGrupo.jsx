...
const SeleccionGrupos = () => {
  ...
  const obtenerGrupos = async () => {
    try {
      const response = await axios.get(`http://192.168.200.5:8000/api/group/zone/${ZoneAux}`); //ip de mi casa
  ...
  const obtenerBrigadistas = async () => {
    try {
      const response = await axios.get('http://192.168.200.5:8000/api/brigadista/all');
  ...
  const unirseAGrupo = async () => {
    try {
      const memberID = "65e7b8e4e151d9a9684db1a4"; // Reemplaza 'ID_DEL_BRIGADISTA' con el ID del miembro
      await axios.put(`http://192.168.200.5:8000/api/group/brigadista/${selectedGroup._id}`, { members: memberID });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.locationContainer}>
        ...
        {/* Mostrar coordenadas */}
        <Text style={styles.coordenadas}>{coordenadas.latitude}, {coordenadas.longitude}</Text>
      ...
      <Text style={styles.title}>Brigadistas:</Text>
        {selectedGroup && selectedGroup.members ? (
          selectedGroup.members.map((brigadistaId, index) => {
            ...
            return <Text key={index}>{brigadista ? brigadista.firstName+" "+brigadista.lastName : 'Brigadista desconocido'}</Text>;
          })
        ) : (

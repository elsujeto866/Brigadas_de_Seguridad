...
function CrearGrupo() {
  ...
  const puntos = [
    { id: 1, position: [-0.266902, -78.53253], name: "Punto 1" },
    { id: 2, position: [-0.270406, -78.516898], name: "Punto 2" },
    { id: 3, position: [-0.278087, -78.52431], name: "Punto 3" },
  ...
  useEffect(() => {
    if (fechaSeleccionada) {
      ...
      axios
        .get(`http://localhost:8000/api/horarios/${fechaAuxiliar}`) // Cambia la URL según tu configuración
        .then((response) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Box className="formato-crear-grupo-container">
        {" "}
        {/* Aplica una clase al contenedor */}
        <p className="label">Nombre de Grupo:</p>{" "}
        {/* Aplica una clase al elemento de texto */}
        <TextField
          type="text"
          value={nombreGrupo}
